...
Cluster provisioning usually takes less than 10 minutes. You can query the status of your cluster with the following command. When your cluster status is `ACTIVE`, you can proceed.
    ...
    $ aws eks describe-cluster --name k8s-cluster-name --query cluster.status --output text
...
Once the cluster has moved to the `ACTIVE` state, download and run the `create-kubeconfig.sh` script. You'll need to modify the script before executing it to include the proper cluster name

    aws s3 cp s3://aws-kubernetes-artifacts/v0.5/create-kubeconfig.sh .

Modify create-kubeconfig.sh

Change the following lines to reflect your cluster name

    'sed -i -e "s#<endpoint-url>#$(aws eks describe-cluster --name k8s-workshop'
    'sed -i -e "s#<base64-encoded-ca-cert>#$(aws eks describe-cluster --name k8s-workshop'
    'sed -i -e "s#<cluster-name>#k8s-workshop'
...
- Stack name so that it is unique (on line 2)
- ClusterName value on line 7 to reflect your cluster name.
- NodeGroupName on line 8 so that it is unique
...
Update line 12 to reflect the stack name you are using for your worker nodes.
